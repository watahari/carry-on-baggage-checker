...
function generateCompatibilityReport(suitcase, results) {
    ...
    const compatibilityRate = totalAirlines === 0 ? 0 : (results.compatible.length / totalAirlines) * 100;
    
    const regionBreakdown = categorizeAirlinesByRegion(results.compatible);
    
    const restrictionAnalysis = {
        weightIssues: results.incompatible.filter(a => 
            suitcase.weight && a.restrictions && a.restrictions.weight && suitcase.weight > a.restrictions.weight
        ).length,
        dimensionIssues: results.incompatible.filter(a => 
            a.restrictions && (
        ...
        lengthIssues: results.incompatible.filter(a => 
            a.restrictions && a.restrictions.length && suitcase.totalLength > a.restrictions.length
        ).length
    };
